...
knitr::include_graphics(here::here("book/images/map_sites.png"))
```
...
# For demo, we randomly allocate each observation into one of five clusters.
df_demo <- dfs
df_demo$cluster <- sample(1:5, nrow(df_demo), replace = TRUE)

group_folds_train <- purrr::map(
  seq(length(unique(df_demo$cluster))),
  ~ {
    df_demo |>
      select(cluster) |>
      mutate(idx = 1:n()) |>
